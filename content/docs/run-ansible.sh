...
# Ansible Installation and Configuration Script
# See http://blog.trifork.com/2013/03/26/ansible-next-generation-configuration-management/
...
sudo geany /etc/ansible/ansible.cfg
...
#list info
ansible -i hosts GDNCSWMGRQA0015.albandrieu.com -m setup
...
# Inventory
sudo pip3.9 install --upgrade ansible-cmdb
# If not python2.7
...
# Upon failure
rm -Rf /home/albandrieu/.local/bin/ansible-lint
rm -Rf /home/albandrieu/.pyenv/shims/ansible-lint
which ansible-lint

ansible-lint --write ./

ansible-lint -v -c .ansible-lint
# mega linter is doings
ansible-lint -v --format sarif
ansible-lint --offline --nocolor -f codeclimate "./devenv.yaml"

ansible-builder build --tag=my-custom-ee --verbosity 2 # --container-runtime=docker
ansible-builder create

ansible-builder introspect --sanitize ./../.direnv/

python3 -m pip install ansible-navigator --user
ansible-navigator images
ansible-navigator collections
ansible-navigator collections --container-engine podman --execution-environment false
ansible-navigator config

ansible-runner run ./ --container-image my-custom-ee:latest -p playbooks/consul.yml --inventory envs/dev/inventory.ini -vvvv
...
ln ~/.ansible_vault_password .
ANSIBLE_VAULT_PASSWORD_FILE=.ansible_vault_password
#ansible-navigator run ./ --container-image my-custom-ee:latest -p playbooks/consul.yml -i envs/dev/inventory.ini --skip-tags aws,docker,init,nomad,vault,check_vault --limit gra1vaultsrvdev1 -vvvv --check
ansible-navigator run playbooks/consul.yml --inventory envs/dev/inventory.ini --skip-tags aws,docker,init,nomad,vault,check_vault --limit gra1vaultsrvdev1 -vvvv --check --container-engine podman --execution-environment false
# --mode interactive

# Disable venv virtualenv
rm -Rf ../.venv/ ../.direnv/
...
ansible-playbook playbooks/consul.yml --inventory envs/dev/inventory.ini --skip-tags aws,docker,init,nomad,vault,check_vault --limit gra1vaultsrvdev1 -vvvv

ansible-vault edit envs/prod/group_vars/all/vault.yml
ansible-vault edit envs/prod/group_vars/web_front/vault.yml

sudo mkdir /var/log/ansible/
sudo chmod 777 /var/log/ansible/
ansible-lint --generate-ignore
ls -lrta .ansible-lint-ignore

# Mandatory variable 'ansible_default_ipv4' not defined
# See https://medium.com/opsops/ansible-default-ipv4-is-not-what-you-think-edb8ab154b10
# ip r
ip route
# Remove the --limit

sudo snap install semaphore
sudo semaphore user add --admin --name "AlbanAndrieu" --login nabla --email alban.andrieu@free.fr --password XXX

echo 'source <(semaphore completion bash)' >>~/.bashrc

# Create vault-pass as Key Store login with password
# https://github.com/ansible-semaphore/semaphore/issues/919
# --vault-password-file=/media/albanandrieu/data/jm/infra/hashistack-cluster/ansible/.ansible_vault_password

# See http://localhost:3000/auth/login

# The error was: Conditional is marked as unsafe, and cannot be evaluated.
geany /media/albanandrieu/data/jm/github/ansible-nomad/defaults/main.yml
# CHANGE
# - ansible_distribution_version is version('{{ version_to_compare }}', '>')
- ansible_distribution_version is version(version_to_compare, '>')

# Fix for (ansible.legacy.setup) is missing interpreter line
...
# I disable direnv allow and started again
