...
# https://juicefs.medium.com/deploy-and-use-juicefs-to-store-data-on-digitalocean-c3c44decfd01
ps -edf | grep juicefs
...
# juicefs gc  redis://juicefs-redis.service.gra.${NOMAD_VAR_env}.consul:6380/3
...
# Warning upgrade could make use believe that metrics disappeared, but they appeared at mount time,
# So re apply csi-juicefs.tf
# Stop the job using the juicefs storage and restart it so that it can re mount the storage with metrics enabled

# nomad volume status -namespace=*
# nomad volume deregister -namespace=datascience juicefs-gra-citemap-${NOMAD_VAR_env}
# aws --endpoint-url https://s3.gra.perf.cloud.ovh.net --profile s3-${NOMAD_VAR_env} s3 rm s3://juicefs-gra-citemap-${NOMAD_VAR_env} --recursive --exclude "*juicefs_uuid"
